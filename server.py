...
def predict_caffe(frame_files):
    ...
    for cidx, chunk in enumerate(slice_in_chunks(frame_files, batch_size)):
        data = np.zeros((len(chunk), 3, net.blobs['data'].data.shape[2], net.blobs['data'].data.shape[3]))
...
def get_prediction(file_path):
    ...
    for index, row in enumerate(predictions):
        ...
        for i in five_best:
            pred_per_label.append({"label": LABEL_MAPPING[i], "prob": row[i].item()})
